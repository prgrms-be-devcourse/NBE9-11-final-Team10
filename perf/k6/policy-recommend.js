...
const age = Number(__ENV.POLICY_AGE || 25);
const region = __ENV.POLICY_REGION || '서울';
const category = __ENV.POLICY_CATEGORY || '금융･복지･문화';
const keyword = __ENV.POLICY_KEYWORD || '대출';
const query = __ENV.POLICY_QUERY || '대학생인데 월세나 전세자금 대출을 지원받고 싶어요.';
...
export default function () {
  ...
  const search = group('2. search policies', () => http.get(
    `${baseUrl}/api/v1/youth-policies/search?age=${age}&region=${encodeURIComponent(region)}&category=${encodeURIComponent(category)}&keyword=${encodeURIComponent(keyword)}&page=0&size=10`,
  ...
  const recommend = group('4. recommend policies', () => http.post(
    ...
    JSON.stringify({ age, region, category, query }),
    {
      headers: jsonHeaders(),
