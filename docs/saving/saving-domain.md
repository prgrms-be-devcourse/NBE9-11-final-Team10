...
예/적금은 자동이체되는 적금 납입 흐름 이외에 추가 입금 불가 상품으로 정의한다.

따라서 일반 송금을 통한 예/적금 전용 Account 수취를 허용하지 않는다.

예금은 가입 시 한 번만 입금된다.

적금은 가입 시 1회차 납입과 자동이체로만 입금된다.

일반 송금이나 일반 입금(topUp)으로 예/적금 전용 Account에 임의 입금할 수 없다.

이 정책을 두는 이유는 `Account.balance`와 예/적금 계약 정보가 어긋나는 것을 막기 위해서다.

예를 들어 예금에 일반 송금 입금을 허용하면 `Deposit.principal`과 예금 Account 잔액이 달라질 수 있다.

적금도 일반 송금 입금을 허용하면 `paidAmount`, `nextPaymentDate`, `targetAmount` 기준과 Account 잔액이 맞지 않을 수 있다.
...
일반 송금 수취를 막는 이유는 “예/적금은 자동이체되는 적금 납입 흐름 이외에 추가 입금 불가 상품”이라는 정책을 지키기 위해서다.
