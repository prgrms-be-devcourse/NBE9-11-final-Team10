...
2. 사용자를 조회한다.
3. 본인인증 완료 여부를 확인한다.
4. 중복되지 않는 계좌번호를 생성한다.
5. 잔액 0원, 상태 ACTIVE로 Account를 저장한다.
```
...
| 기능 | 정책 |
| --- | --- |
| 비밀번호 설정 | 활성 계좌만 가능, 이미 설정되어 있으면 실패 |
...
송금처럼 두 계좌가 함께 바뀌는 경우 데드락 위험을 줄이기 위해 ID가 작은 계좌부터 락을 잡는다.
