...
public class InstallmentPaymentScheduler {
    ...
    private static final String LOCK_KEY = "lock:saving:installment-payment";
    ...
    public void processDueInstallmentPayments() {
        try {
            distributedLockTemplate.executeWithLock(
                    ...
                    () -> {
                        int processedCount =
                                savingDepositService.processDueInstallmentPayments();
                        log.info("적금 정기 자동이체 스케줄러 실행 완료. processedCount={}", processedCount);
            ...
            {
                log.info("적금 정기 자동이체 스케줄러 실행 건너뜀. reason=lock_not_acquired");
                return;
            }
